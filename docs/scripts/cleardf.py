...
def adapt_data(df,colname="moves"):
	...
	print(cols,_)
	matrix=[]
	cols_names=["opening_name"]+[i for i in range(cols)]
...
def main():
	df=pd.read_csv("chess_openings3movestransalted.csv")#.head(10)
	data,cols=adapt_data(df,colname="moves")
	ndf = pd.DataFrame(data, columns =cols);print(ndf) 
	ndf.to_csv("chess_openings5individualcolumnsintegers.csv")
